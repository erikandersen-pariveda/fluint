package org.flexunit.runners {
	...
	public class FlexUnit4Runner extends EventDispatcher implements IRunner {
...
*/
         ...
         * @private
         */
		public function getNextTestDescription():Description {
			var description:Description;
			if ( !cursor.afterLast && !cursor.beforeFirst ) {
				description = cursor.current as Description;
				cursor.moveNext();
         ...
         * @private
         */
		private function buildTestCollection( descriptions:Array ):ArrayCollection {
			var testMethods:ArrayCollection;

			testMethods = new ArrayCollection( descriptions );
...
//------------------------------------------------------------------------------
		...
		private function getOrderValueFromMethod( method:XML, metadata:String ):int {
			...
			var orderString:String = MetadataTools.getArgValueFromMetaDataNode( method, metadata, "Order" );
			if ( orderString ) {
		...
		private function filterOutIgnore( method:XML ):Boolean {
			return !( MetadataTools.nodeHasMetaData( method, "Ignore" ) );
		}
...
/* 			var request:IRequest;
			trace("Running some test " + request.description.displayName );
			var className:String = request.description.testClassOrInstance;
			var clazz:Class = getDefinitionByName( className ) as Class;
			this.testObject = new clazz();

			RunnerLocator.getInstance().registerRunnerForTest( testObject, this );

			typeInfo = describeType( testObject );

			var methodList:XMLList;			
			methodList = MetadataTools.getMethodsList( typeInfo );
			
			var beforeClassList:XMLList = MetadataTools.getMethodsDecoratedBy( methodList, "BeforeClass" );
			var afterClassList:XMLList = MetadataTools.getMethodsDecoratedBy( methodList, "AfterClass" );

			//need to sort these two by any potential order in the metadata
			var beforeList:XMLList = MetadataTools.getMethodsDecoratedBy( methodList, "Before" );
			...
			var afterList:XMLList = MetadataTools.getMethodsDecoratedBy( methodList, "After" );
			...
			var filterList:XMLList = MetadataTools.getMethodsDecoratedBy( methodList, "Filter" );
			var sortList:XMLList = MetadataTools.getMethodsDecoratedBy( methodList, "Sort" );

			//Tests to run will be this list, minus the Ignore list, minus any removed by any filter functions
			var ignoreList:XMLList = MetadataTools.getMethodsDecoratedBy( methodList, "Ignore" );

			//needs revision
			testCollection = buildTestCollection( request.description.children.source );
			testCollection.sort = sorter;
			testCollection.filterFunction = filter;
			testCollection.refresh();
 ...
 */		}
		...
		public function FlexUnit4Runner() {
			if (!sorter) {
